...
- Goleador, balón de oro y equipo sorpresa son a nivel jugador/subjetivos → **entrada manual**
  (no hay datos de jugadores para modelarlos).
...
- **Agregado por equipo** (una fila por selección con `team, xg_for, xg_against`).
- **Por partido** (una fila por encuentro con `date, home_team, away_team, home_xg, away_xg`,
  típico de exportes con muchas ligas): el cargador filtra automáticamente a los partidos
  **entre selecciones**, aplica el corte de recencia (`CORTE_RECIENTE`) y pondera por
  **decaimiento temporal** (`XG_DECAY_PER_DAY`) antes de promediar. Genera el archivo con
  `python scripts/setup_xg.py` (lee `~/Downloads/xG.csv` por defecto).
...
El optimizador saca **+0.04 pts/partido** sobre elegir el marcador más probable (modela 1X2
con LightGBM: log-loss 0.86 / accuracy 0.60 / ECE 0.03).
